<x-admin-layout>
    ...
    <div class="py-12" x-data="{ 
                    ...
                    <div class="mb-4 flex justify-between items-center">
                        <div class="space-x-2">
                            <a href="{{ route('admin.departments.create') }}">
                                <x-primary-button>{{ __('Thêm Phòng Ban') }}</x-primary-button>
                            </a>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($departments as $department)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-500">{{ Str::limit($department->description, 100) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-blue-600 font-bold cursor-pointer hover:underline"
                                            @click="currentDept = '{{ $department->name }}'; employees = {{ $department->employees->toJson() }}; showList(); openModal = true">
                                            {{ $department->employees->count() }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                            <a href="{{ route('admin.departments.edit', $department) }}" class="text-blue-600 hover:text-blue-900">Sửa</a>
                                            <form action="{{ route('admin.departments.destroy', $department) }}" 
                                                method="POST" 
                                                class="inline-block confirm-form"
                                                data-title="Xóa phòng ban?"
                                                data-text="Bạn có chắc chắn muốn chuyển phòng '{{ $department->name }}' vào thùng rác?">
                                                @csrf @method('DELETE')
                                ...
                                @empty
                                    <tr><td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">Chưa có phòng ban nào.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
        ...
        {{-- Modal Đa năng (Danh sách & Chi tiết) --}}
                ...
                <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    ...
                    <div x-show="viewMode === 'list'" class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">
                            Nhân viên phòng: <span x-text="currentDept" class="text-blue-600"></span>
                        </h3>
                            ...
                            <div class="text-center">
                                ...
                                <div class="text-left space-y-3 bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <div class="flex justify-between text-sm">
                                        ...
                                        <span class="font-medium" x-text="currentDept"></span>
                                    </div>
                                </div>
                            </div>
                        </template>
