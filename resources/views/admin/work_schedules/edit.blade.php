<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Sửa Ca Làm Việc') }}: {{ $workSchedule->name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('admin.work-schedules.update', $workSchedule->id) }}">
                        @csrf
                        ...
                        <div>
                            ...
                            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name"
                                :value="old('name', $workSchedule->name)" required autofocus />
                        ...
                        <div class="mt-4">
                            ...
                            {{-- Format lại giờ để hiển thị đúng trong input type="time" --}}
                            <x-text-input id="start_time" class="block mt-1 w-full" type="time" name="start_time"
                                :value="old('start_time', \Carbon\Carbon::parse($workSchedule->start_time)->format('H:i'))" required />
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="end_time" class="block mt-1 w-full" type="time" name="end_time"
                                :value="old('end_time', \Carbon\Carbon::parse($workSchedule->end_time)->format('H:i'))"
                                required />
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="late_threshold" class="block mt-1 w-full" type="time"
                                name="late_threshold" :value="old('late_threshold', $workSchedule->late_threshold ? \Carbon\Carbon::parse($workSchedule->late_threshold)->format('H:i') : '')" />
