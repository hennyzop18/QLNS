<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Sửa Lịch Làm Việc Đã Gán cho') }}: {{ $employee->full_name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST"
                        action="{{ route('admin.employees.schedules.update', [$employee, $schedule_assignment]) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="work_schedule_id" id="work_schedule_id"
                                ...
                                @foreach($workSchedules as $schedule)
                                    <option value="{{ $schedule->id }}" @selected(old('work_schedule_id', $schedule_assignment->work_schedule_id) == $schedule->id)>
                                        {{ $schedule->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="start_date" class="block mt-1 w-full" type="date" name="start_date"
                                :value="old('start_date', $schedule_assignment->start_date->format('Y-m-d'))"
                                required />
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="end_date" class="block mt-1 w-full" type="date" name="end_date"
                                :value="old('end_date', $schedule_assignment->end_date?->format('Y-m-d'))" />
